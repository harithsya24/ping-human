...
The conversation history below contains the COMPLETE context. Use it ALL. Identify the topic and stick to it."""
    ...
    # Build messages with FULL conversation history for maximum context awareness
    messages = [{"role": "system", "content": system_prompt}]
    
    # Use ALL available conversation history - GPT-4o-mini supports 128k tokens
    # This ensures complete context awareness across the entire conversation
    history_to_use = conversation_history[chat_id]
    
    # For very long conversations (200+ messages), use intelligent context windowing:
    # - Keep first 10 messages (conversation foundation)
    # - Keep middle section summary if needed
    # - Keep last 150 messages (recent context)
    if len(history_to_use) > 200:
        # For extremely long conversations, preserve key context
        history_to_use = (
            history_to_use[:10] +  # Conversation start
            history_to_use[-150:]  # Recent 150 messages
        )
    elif len(history_to_use) > 100:
        # For long conversations, keep start + recent
        history_to_use = (
            history_to_use[:10] +  # Conversation start
            history_to_use[-90:]   # Recent 90 messages
        )
    # For conversations < 100 messages, use ALL history (full context)
    
    # Add ALL selected history to maintain complete context awareness
